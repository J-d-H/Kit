...
function load(repository, kha, element) {
    ...
    function create(callback) {
        var child = cp.spawn(config.projectsDirectory() + "/" + repository + "/Kha/Tools/hake/" + exe, [getSystem(select), "mp3=" + config.mp3Encoder(), "aac=" + config.aacEncoder()], { cwd: config.projectsDirectory() + "/" + repository });
            ...
            switch (getSystem(select)) {
                ...
                case 'windowsrt':
                    ...
                    if (!fs.existsSync(projectFile))
                        projectFile = config.projectsDirectory() + '/' + repository + '/build/' + getSystem(select) + '-build/' + projectName + '.sln';
                    open(projectFile);
                    break;
                case 'osx':
                case 'ios':
                    ...
                    if (!fs.existsSync(projectFile))
                        projectFile = config.projectsDirectory() + '/' + repository + '/build/' + getSystem(select) + '-build/' + projectName + '.xcodeproj';
                    open(projectFile);
                    break;
                case 'wpf':
                case 'xna':
...
exports.load = load;
//# sourceMappingURL=Kit/../projectPages/project.js.map
